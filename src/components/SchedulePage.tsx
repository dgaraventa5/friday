...
function groupTasksByStartDate(tasks: Task[]): Record<string, Task[]> {
  return tasks.reduce((acc, task) => {
    ...
    const startDateObj = startOfDay(new Date(task.startDate));
    // Use local date string (YYYY-MM-DD) instead of ISO string to avoid timezone issues
    const key = startDateObj.toLocaleDateString('en-CA'); // en-CA uses YYYY-MM-DD format
...
export function SchedulePage({ tasks, onToggleComplete, onEdit }: SchedulePageProps) {
  ...
  const sortedDates = Object.keys(grouped)
    .filter(dateKey => {
      ...
      return !isBefore(startOfDay(dateObj), today) || dateKey === todayKey;
    })
    .sort();
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-neutral-900 mb-4 font-sans flex items-center gap-2">
        <span role="img" aria-label="calendar">🗓️</span> Full Task Schedule
      </h1>
      {sortedDates.length === 0 && (
        <div className="text-center py-12 text-neutral-600 font-sans">No scheduled tasks yet.</div>
      )}
      {sortedDates.map(dateKey => {
        ...
        console.log('Rendering date:', dateKey, 'as Date object:', dateObj, 'formatted:', format(dateObj, 'EEE MMM d yyyy'));
        
        return (
          <div key={dateKey} className="bg-white rounded-2xl shadow-card p-4 md:p-6">
