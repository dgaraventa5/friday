...
export function TaskList({ 
  ...
  const completedTasks = showCompletedInline
    ...
    : tasks.filter(task => {
        ...
        const taskStartDateString = startOfDay(new Date(task.startDate)).toLocaleDateString('en-CA');
        return taskStartDateString === todayDateString;
      });
      
  const allTasksCompleted = incompleteTasks.length === 0 && completedTasks.length > 0;
  ...
  if (tasks.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-neutral-900 mb-2 font-sans">No tasks yet</h3>
        <p className="text-neutral-600 font-sans">Add your first task to get started with focused productivity.</p>
      </div>
    );
  }
  ...
  if (allTasksCompleted && showCompletionCelebration) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-2xl font-bold text-neutral-900 mb-2 font-sans">All done! 🎉</h3>
        <p className="text-lg text-neutral-600 mb-4 font-sans">You've completed all your tasks for today.</p>
        <div className="bg-gradient-to-r from-success-500/10 to-primary-100 rounded-lg p-4 max-w-md mx-auto font-sans">
          ...
          <p className="text-xs text-neutral-500 mt-2 font-sans">— Mark Twain</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-neutral-900 font-sans">{title}</h2>
      </div>
