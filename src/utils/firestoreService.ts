...
export async function loadTasksFromFirestore(userId: string): Promise<Task[]> {
  try {
    ...
    // If no tasks in Firestore but we have local tasks, use those
    if (tasks.length === 0) {
...
export async function saveCategoriesToFirestore(
  ...
  try {
    ...
    const userCategoriesRef = doc(db, COLLECTIONS.CATEGORIES, userId);
    await setDoc(userCategoriesRef, {
      categories: prepareForFirestore(categories),
      userId,
    });
    console.log('[Firestore] Categories saved successfully');
...
export async function loadCategoriesFromFirestore(
  ...
  try {
    ...
    console.log(
      '[Firestore] Categories collection path:',
      COLLECTIONS.CATEGORIES,
    );

    const userCategoriesRef = doc(db, COLLECTIONS.CATEGORIES, userId);
    console.log(
      '[Firestore] Categories document reference:',
      userCategoriesRef.path,
    );
    ...
    if (docSnap.exists()) {
      console.log('[Firestore] Raw document data:', docSnap.data());
    }
    ...
    if (!docSnap.exists()) {
      ...
      console.log(
        '[Firestore] Local categories found:',
    ...
    console.log('[Firestore] Document data after timestamp conversion:', data);

    // Extract categories array from document data
    if (data.categories && Array.isArray(data.categories)) {
      console.log(
        `[Firestore] Found ${data.categories.length} categories in array`,
      );
      return data.categories as Category[];
    } else {
      console.log('[Firestore] No categories array found in document data');
      return [];
    }
  } catch (error) {
...
export async function loadOnboardingStatusFromFirestore(
  ...
  try {
    ...
    if (!docSnap.exists()) {
      console.log('[Firestore] No onboarding status found');
      // Try localStorage
    ...
    console.log(
      `[Firestore] Onboarding status: ${data.onboarding_complete === true}`,
    );
