...
describe('assignStartDates', () => {
  const workCategory: Category = { id: '1', name: 'Work', color: '#0000ff', dailyLimit: 10, icon: '💼' };
  const homeCategory: Category = { id: '2', name: 'Home', color: '#00ff00', dailyLimit: 3, icon: '🏠' };
  const healthCategory: Category = { id: '3', name: 'Health', color: '#ff0000', dailyLimit: 3, icon: '💪' };

  function makeTask(id: string, category: Category, estimatedHours: number): Task {
    return {
      id,
      name: `Task ${id}`,
      category,
  ...
  it('respects daily cap and category caps', () => {
    ...
    const day1 = scheduled.filter(t => startOfDay(t.startDate).getTime() === today.getTime());
    expect(day1.length).toBeLessThanOrEqual(4);
    const workHours = day1.filter(t => t.category.name === 'Work').reduce((sum, t) => sum + t.estimatedHours, 0);
    expect(workHours).toBeLessThanOrEqual(10);
    const homeHours = day1.filter(t => t.category.name === 'Home').reduce((sum, t) => sum + t.estimatedHours, 0);
    expect(homeHours).toBeLessThanOrEqual(3);
    const healthHours = day1.filter(t => t.category.name === 'Health').reduce((sum, t) => sum + t.estimatedHours, 0);
  ...
  it('skips Work tasks on weekends', () => {
    ...
    const workTasks = Array.from({ length: 12 }, (_, i) => makeTask(`${i + 1}`, workCategory, 2));
    const scheduled = assignStartDates(workTasks, 4, friday);
    // Friday: 4 tasks
    const fri = scheduled.filter(t => startOfDay(t.startDate).getTime() === friday.getTime());
    expect(fri.length).toBe(4);
    // Saturday: should have 0 Work tasks
    const sat = addDays(friday, 1);
    const satTasks = scheduled.filter(t => startOfDay(t.startDate).getTime() === sat.getTime());
    expect(satTasks.filter(t => t.category.name === 'Work').length).toBe(0);
    // Sunday: should have 0 Work tasks
    const sun = addDays(friday, 2);
    const sunTasks = scheduled.filter(t => startOfDay(t.startDate).getTime() === sun.getTime());
    expect(sunTasks.filter(t => t.category.name === 'Work').length).toBe(0);
    // Monday: should have 4 Work tasks
    const mon = addDays(friday, 3);
    const monTasks = scheduled.filter(t => startOfDay(t.startDate).getTime() === mon.getTime());
  ...
  it('overflows tasks to future days', () => {
    ...
    const baseDate = startOfDay(new Date());
    const tasks = Array.from({ length: 10 }, (_, i) => makeTask(`${i + 1}`, homeCategory, 1));
    const scheduled = assignStartDates(tasks, 4, baseDate);
    // Should be spread over 4 days (3, 3, 3, 1)
    const day1 = scheduled.filter(t => startOfDay(t.startDate).getTime() === baseDate.getTime());
    const day2 = scheduled.filter(t => startOfDay(t.startDate).getTime() === addDays(baseDate, 1).getTime());
    const day3 = scheduled.filter(t => startOfDay(t.startDate).getTime() === addDays(baseDate, 2).getTime());
    const day4 = scheduled.filter(t => startOfDay(t.startDate).getTime() === addDays(baseDate, 3).getTime());
...
function createTestTask(id: string, name: string, category: string = 'Work', daysFromNow: number = 0): Task {
...
describe('taskPrioritization', () => {
  describe('assignStartDates', () => {
    ...
    it('should handle tasks with undefined startDate', () => {
      ...
      const expectedDateString = startOfDay(task.dueDate).toISOString().split('T')[0];
    ...
    it('should consistently filter today\'s tasks', () => {
      ...
      const todayTasks = result.filter(t => {
        const taskDateString = startOfDay(new Date(t.startDate)).toISOString().split('T')[0];
